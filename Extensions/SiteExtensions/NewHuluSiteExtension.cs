...
{
    class NewHuluSiteExtension : BaseSiteExtension
    {
        public override bool Filter(string url)
        {
            return Regex.IsMatch(url, @"^(?:https?:\/\/)?(?:www\.)?new\.hulu\.com\/(?:watch\/([{(]?[0-9A-F]{8}[-]?(?:[0-9A-F]{4}[-]?){3}[0-9A-F]{12}[)}]?))|live-tv$", RegexOptions.IgnoreCase);
        }

        public override string Name => "Hulu (Beta)";
